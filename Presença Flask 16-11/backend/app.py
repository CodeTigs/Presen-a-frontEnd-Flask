...
# Nova rota para calcular produtividade
@app.route('/api/calcular-produtividade', methods=['POST'])
def calcular_produtividade():
    data = request.get_json()
    
    # 1. Validação simples
    if not data or 'horas_trabalhadas' not in data or 'tarefas_concluidas' not in data:
        return jsonify({'erro': 'Dados incompletos'}), 400
        
    horas = float(data['horas_trabalhadas'])
    tarefas = int(data['tarefas_concluidas'])
    
    # 2. Lógica de Negócio (Tratamento)
    if horas <= 0:
        return jsonify({'mensagem': 'Horas devem ser maiores que zero'}), 400
    ...
    # 3. Resposta
    return jsonify({
        'tarefas_por_hora': round(ratio, 2),
        'nivel_produtividade': status,
        'mensagem': f'Você completou {tarefas} tarefas em {horas}h.'
    })
