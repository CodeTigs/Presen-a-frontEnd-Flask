...
function setupEventListeners() {
    ...
    // Listener para a nova funcionalidade
const formProd = document.getElementById('formProdutividade');
if (formProd) {
    formProd.addEventListener('submit', handleCalculoProdutividade);
}
}
...
async function handleCalculoProdutividade(e) {
    e.preventDefault();
    
    const horas = document.getElementById('inputHoras').value;
    const tarefas = document.getElementById('inputTarefas').value;
    const resultDiv = document.getElementById('resultadoProdutividade');
    
    try {
        // O Request para o Flask
        // Nota: API_BASE_URL já está definido no início do seu script.js como http://localhost:5000
        const response = await fetch(`${API_BASE_URL}/api/calcular-produtividade`, {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            document.getElementById('msgResultado').textContent = data.mensagem;
            document.getElementById('statusResultado').textContent = `Nível: ${data.nivel_produtividade} (${data.tarefas_por_hora} tarefas/h)`;
            document.getElementById('statusResultado').style.color = data.nivel_produtividade === 'Excelente' ? 'var(--success-color)' : 'var(--primary-color)';
            
            showNotification('Cálculo realizado com sucesso!', 'success');
        } else {
            showNotification(data.erro || 'Erro ao calcular', 'error');
        }
